    ...
    if ex_adjust:
        ...
        for mult in multiplier-1:
            if mult != 0:
                ...
                tr_list = np.append(tr_list,np.tile(np.append(a[:tr_len[i]],i*np.ones((tr_len[i],1),dtype = np.int32),axis=1),(mult, 1)),axis=0)
                val_list = np.append(val_list,np.tile(np.append(a[tr_len[i]:],i*np.ones((val_len[i],1),dtype = np.int32),axis=1),(mult, 1)),axis=0)
...
class ex_create(keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        X, y = self.__data_generation(idx_start)
