...
train_dict = {
    ...
    'epochs'        : 100,       # number of epochs we run on each training ensemble/mini-batch
    ...
    'opt_patience'  : 100,       # number of epochs with the same accuracy before force breaking the training ensemble/mini-batch
    ...
    'save_location' : 'F3/100_epochs_80000_examples'    # file name for the saved trained model
}

# Define the dictionary holding all the prediction parameters
pred_dict = {
    'keras_model'   : None, #keras.models.load_model('F3/100_epochs_80000_examples.h5'), # input model to be used for prediction, to load a model use: keras.models.load_model('write_location')
    ...
    'save_location' : 'predictions/F3_test',       # file name for the saved prediction
